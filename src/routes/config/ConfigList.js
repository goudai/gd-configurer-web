...
import UserModifyUserRankModal from './UserModifyUserRankModal'
const Column = Table.Column

function UserList({dispatch, modalType, list, loading, total, query, user}) {


    const showModifyUserRankModalHandler = (user) => {
      console.log(user);
      dispatch({
        type: 'user/showModifyUserRankModal',
        payload: user
      })
    }
    ...
    return (
      ...
      <div>
        ...
        <UserModifyUserRankModal
          dispatch={dispatch}
          visible={modalType === 'modifyUserRank'}
          user={user}
          loading={loading}
        />
        ...
        >
          ...
          <Column title="操作" render={(text, record) => (
              <span>
                ...
                <a href="javascript:;" onClick={showModifyUserRankModalHandler.bind(null, record)}>修改等级</a>
                <span className="ant-divider"/>
                <Dropdown
                  overlay={
                    <Menu>
